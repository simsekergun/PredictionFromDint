...
save ./files_mat_results/N_vs_w_h_error_SM1_50.mat minerrorsW maxerrorsW meanerrorsW minerrorsH maxerrorsH meanerrorsH Ns center1s center2s center3s
...
for k = 1:8
    subplot(8,3,1+(k-1)*3)
    scatter(center1s(k,:), center2s(k,:), csizee, maxerrorsW(k,:), 'filled');
    c = colorbar;  % clim([75 100]);
    axis tight
    xlabel('N_{c,1}');
    ylabel('N_{c,2}');
    grid on;
    c.Label.String = 'Max. Error (nm)';
    c.Label.Rotation = 270; % For a vertical label
    subplot(8,3,2+(k-1)*3)
    scatter(center3s(k,:), center2s(k,:), csizee, maxerrorsW(k,:), 'filled');
    axis tight
    c = colorbar; % clim([75 100]);
    xlabel('N_{c,3}');
    ylabel('N_{c,2}');
    ...
    grid on;
    subplot(8,3,3+(k-1)*3)
    scatter(center1s(k,:), center3s(k,:), csizee, maxerrorsW(k,:), 'filled');
    axis tight
    c = colorbar; %  clim([75 100]);
    xlabel('N_{c,1}');
    ylabel('N_{c,3}');
...
for k = 1:8
    subplot(8,3,1+(k-1)*3)
    scatter(center1s(k,:), center2s(k,:), csizee, meanerrorsW(k,:), 'filled');
    c = colorbar;  % clim([75 100]);
    axis tight
    xlabel('N_{c,1}');
    ylabel('N_{c,2}');
    ...
    c.Label.Rotation = 270; % For a vertical label
    subplot(8,3,2+(k-1)*3)
    scatter(center3s(k,:), center2s(k,:), csizee, meanerrorsW(k,:), 'filled');
    axis tight
    c = colorbar; % clim([75 100]);
    xlabel('N_{c,3}');
    ylabel('N_{c,2}');
    ...
    grid on;
    subplot(8,3,3+(k-1)*3)
    scatter(center1s(k,:), center3s(k,:), csizee, meanerrorsW(k,:), 'filled');
    axis tight
    c = colorbar; %  clim([75 100]);
    xlabel('N_{c,1}');
    ylabel('N_{c,3}');
